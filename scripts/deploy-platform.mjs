...
for(const name of ['chainId','registry','feePool','v4PoolManager','platformToken','governance','treasuryRecipient','rewardDistributor','quoteSigner','governanceDelay','assets'])if(c[name]===undefined)throw Error('Missing '+name);
...
for(const name of ['registry','feePool','v4PoolManager','platformToken','rewardDistributor'])if(await provider.getCode(c[name])==='0x')throw Error(name+' must have deployed code');
...
const vault=await new ethers.ContractFactory(artifact.abi,artifact.evm.bytecode.object,signer).deploy(wallet.address,c.feePool,c.v4PoolManager,c.platformToken,c.treasuryRecipient,c.rewardDistributor,c.quoteSigner,c.governanceDelay);
await vault.waitForDeployment();
const output=path.resolve(c.outputFile??`deployment-platform-${c.chainId}-${Date.now()}.json`);
const result={chainId:String(c.chainId),OursPlatformTreasury:vault.target,registry:c.registry,feePool:c.feePool,v4PoolManager:c.v4PoolManager,platformToken:c.platformToken,governance:c.governance,treasuryRecipient:c.treasuryRecipient,rewardDistributor:c.rewardDistributor,status:'deployed; configuration in progress'};
