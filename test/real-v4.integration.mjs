...
async function swap(pool,assetIn,amount=10n**15n,nonce=1,options={}){const p={poolId:pool.id,assetIn,maxAmountIn:amount,minAmountOut:1,sqrtPriceLimitX96:assetIn===pool.key.currency0?LOW:HIGH,purpose:0,deadline:await time()+3600,nonce,signerEpoch:1,...options};return {p,sig:await signed(p,swapTypes)};}
...
before(async()=>{
 ...
 const rewards=await deploy('test/MockRewardDistributor');
 const vault=await deploy('OursPlatformTreasury',[admin.address,fee.target,manager.target,token.target,other.address,rewards.target,signer.address,20]);
...
for(const direction of ['zeroForOne','oneForZero','native'])test(`real-v4: ${direction} swap settles real AMM deltas and isolates operating reserve`,async()=>{
 ...
 await deposit(asset);const supply=await f.token.totalSupply();const x=await swap(pool,asset);await tx(f.vault.executeSwap(x.p,x.sig));const received=supply-await f.token.totalSupply();assert(received>0n&&received<x.p.maxAmountIn);assert.equal(await f.vault.strategyBalance(asset),E*8n/10n-x.p.maxAmountIn);assert.equal(await f.vault.operatingBalance(asset),E/5n);await solvent();
});
...
test('real-v4: contract owns real liquidity, harvests real fees, and removes it only after delay',async()=>{
 await deposit(f.low.target);await deposit(f.stock.target);const x=await swap(f.buy,f.low.target,10n**15n,1,{purpose:1});await tx(f.vault.executeSwap(x.p,x.sig));const add=await lp();await tx(f.vault.addLiquidity(add.p,add.sig));
...
test('real-v4: native platform swap survives hook synchronization of another currency',async()=>{
 await deposit(zero);const x=await swap(f.hookPool,zero,10n**15n,1,{purpose:1});await tx(f.vault.executeSwap(x.p,x.sig));assert((await f.vault.strategyBalance(f.stock.target))>0n);await solvent();
});
...
test('custody regression: governance cannot be renounced because LP exits require it',async()=>{
...
test('real-v4: failed LP minimum restores queued action, position and balances; valid retry succeeds',async()=>{
 ...
 const x=await swap(f.buy,f.low.target,10n**15n,1,{purpose:1});await tx(f.vault.executeSwap(x.p,x.sig));
